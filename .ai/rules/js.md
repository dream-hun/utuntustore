...
## Regenerate Wayfinder with --with-form, or every .form() call breaks
`resources/js/routes` and `resources/js/actions` are gitignored and generated by the vite plugin, which is configured with `formVariants: true` in `vite.config.ts`.

Running `php artisan wayfinder:generate` by hand regenerates them WITHOUT the form variants, and `npm run types:check` then fails with ~14 `Property 'form' does not exist` errors across auth, settings and the two-factor components. Nothing in git shows as changed, so it looks like an unrelated pre-existing break.

Always pass the flag: `php artisan wayfinder:generate --with-form`. Or just run `npm run build` / `npm run dev`, which regenerates through the plugin correctly.
